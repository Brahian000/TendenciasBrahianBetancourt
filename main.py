...
from validator import employeeType, type, userType
import service.LoginService as loginService
from service.Rol import administrativePersonal, doctor, humanResources, nurse, support
...
employee1 = Employee("Julian", "123456789", "julian@example.com", "1234567890", "27/02/2004", "123 Main St", "Recursos Humanos", "juli", "password")
employee2 = Employee("Brahian", "123456789", "julian@example.com", "1234567890", "27/02/2004", "123 Main St", "Administrador", "bra", "password")
employee3 = Employee("doctor", "123456789", "julian@example.com", "1234567890", "27/02/2004", "123 Main St", "Doctor", "doctor", "doctor")
employee4 = Employee("nurse", "123456789", "julian@example.com", "1234567890", "27/02/2004", "123 Main St", "Enfermera", "nurse", "nurse")
...
def isNotNoneUpatedUser(userToUpdate):
    if userToUpdate == None:
        ...
        return
    employeeType.validEmployeeData(userToUpdate)
...
def isNotNoneUpatedPatient(hospital, userToUpdate):
    if userToUpdate == None:
        print("Usuario no encontrado")
        return
    userType.validPatientData(hospital, userToUpdate)
    print("Informacion actualizada con éxito")
...
def adminMenu(hospital, currentUser):
    ...
    while True:
        ...
        if option=="1":
            userType.getPatientData(hospital)
            newPatient = hospital.patients[-1]
            userType.getEmergencyContactData(hospital, newPatient)
            userType.getMedicalInsuranceData(hospital, newPatient)
            userType.allDataCompletePatient(hospital, newPatient)
        elif option=="2":
            ...
            userType.getAppointmetData(hospital, document)
        elif option=="4":
            document = input("Ingrese el documento del paciente:\n")
            userType.getInvoiceData(hospital, document)
        elif option=="5":
...
def HumanResourcesMenu(hospital, currentUser):
    while True:
        ...
        if option=="1":
            employeeType.getEmployeeData(hospital)
        elif option=="2":
            employeeType.getReAssignRolData(hospital)
        elif option=="3":
            deleteEmployee = input("ingrese el nombre de usuario del empleado a eliminar:\n")
            employeeType.getDeleteEmployeeData(hospital, deleteEmployee) 
        elif option=="4":
            userToUpdateName = input("Ingrese el nombre completo del paciente a actualizar:\n")
            userToUpdate = loginService.searchEmployee(hospital, userToUpdateName)
            isNotNoneUpatedUser(userToUpdate)
        elif option == "5":
            employeeType.getAllEmployeesData(hospital)
        elif option == "6":
            print("cerrando sesion...")
            return
        else:
...
def doctorMenu(hospital, currentUser):
    while True:
        ...
        if option=="1":
            userType.getAllPatientData(hospital)
        elif option=="2":
            userType.getClinicalHistoryData(hospital, currentUser.idNumber)
        elif option=="3":
            print("cerrando sesion...")
            return
        else:
...
def nurseMenu(hospital, currentUser):
    while True:
        ...
        if option=="1":
             userType.getAllPatientData(hospital)
        if option=="2":
            userType.getVitalData(hospital)
