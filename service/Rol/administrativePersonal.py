...
def createPatient(hospital, id, fullName, bornDate, genre, address, phoneNumber, email):
    ...
    type.validPhoneNumber(phoneNumber)
    type.validDateAndAge(bornDate)
    if len(address) > 30:
...
def printInvoiceDetails(invoice, doctor):
    ...
    print("Edad del paciente: ", type.getCorrectAge(invoice.patient.bornDate))
    ...
    print("Dias de vigencia de la poliza: ", type.getValidityPolicy(invoice.patient.medicalInsurance.policyValidity))
